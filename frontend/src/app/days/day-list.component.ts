...
import { SkiDay, Bus, Group, Registration, PersonAbsence, ConstraintConfig } from '../core/models';

@Component({
  selector: 'app-day-list',
  standalone: true,
  imports: [FormsModule, MatCardModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatIconModule, MatChipsModule],
  template: `
    <h2>Days & Buses</h2>
...
export class DayListComponent implements OnInit {
  ...
  config = signal<ConstraintConfig | null>(null);
  groups = signal<Group[]>([]);
  ...
  addBus(dayId: string) {
    ...
    const name = (this.newBusNames[dayId] || '').trim() || `${prefix} ${existingBuses.length + 1}`;
    const capacity = this.newBusCapacities[dayId] || this.config()?.default_bus_capacity || 50;
    const reserved = this.newBusReserved[dayId] || this.config()?.default_reserved_seats || 0;
